...
const proxies = [...new Set(fs.readFileSync('/root/1.txt', 'utf-8').replace(/\r/g, '').split('\n'))];

async function tryToSolve(sitekey, host) {

  const setProxy = proxies[~~(Math.random() * proxies.length)];
...
async function solveCaptcha(url, options = {}) {
  const { gentleMode, timeoutInMs = 240000 } = options;
  ...
  while (true) {
    try {
      ...
      if (e.statusCode === 429) {
        console.log('[INFO]: Rate Limited! Wait 30 seconds.. ')
